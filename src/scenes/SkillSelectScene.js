...
export default class SkillSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(W / 2, H * 0.12, '机关全部满级!', { fontFamily: FONTS.title, fontSize: '40px', color: '#ff9f43' }).setOrigin(0.5);
    ...
    this.cards = this.types.map((type, i) => {
      ...
      const name = this.add.text(x, cy - 8, info.name, { fontFamily: FONTS.display, fontSize: '22px', fontStyle: '700', color: '#ffffff' }).setOrigin(0.5);
    ...
    this.confirmText = this.add.text(W / 2, H * 0.88, '选满 3 项', { fontFamily: FONTS.display, fontSize: '24px', fontStyle: '700', color: '#0b0e14' }).setOrigin(0.5);
